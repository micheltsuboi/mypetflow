  ...
  try {
    ...
    nodes.forEach(n => {
      ...
      if (n.type === "n8n-nodes-base.httpRequest" && n.name.includes("Z-API")) {
         n.parameters.url = "={{ ($json.wa_api_url || '').toLowerCase().includes('/token/') ? ($json.wa_api_url.replace(/\/$/, '') + '/send-text') : ($json.wa_api_url.replace(/\/$/, '') + '/token/' + $json.wa_api_token + '/send-text') }}";
