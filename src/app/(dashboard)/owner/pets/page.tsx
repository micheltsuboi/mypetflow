                                            ...
                                            {petSubscriptions.length === 0 ? (
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                    {petSubscriptions.map((sub: any) => {
                                                        ...
                                                        const nextRenewal = sub.next_renewal_date ? new Date(sub.next_renewal_date + 'T12:00:00') : null
                                                        
                                                        return (
                                                            <div key={sub.id} style={{ background: 'var(--bg-tertiary)', padding: '16px', borderRadius: 'var(--radius-lg)', border: `1px solid ${sub.is_active ? 'var(--border)' : 'rgba(239,68,68,0.3)'}`, position: 'relative' }}>
                                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                                    <div style={{ flex: 1 }}>
                                                                    ...
                                                                    {sub.is_active && (
                                                                        <button
                                                                            ...
                                                                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontSize: '0.75rem', fontWeight: 600, padding: '4px' }}
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '1rem' }}>
                    <div style={{ background: 'var(--bg-primary)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', padding: '2rem', width: '100%', maxWidth: '520px', maxHeight: '90vh', overflowY: 'auto' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            {(() => {
                                const plan = subscriptionPlans.find((p: any) => p.id === subSelectedPlanId)
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', marginTop: '1.5rem', paddingTop: '1.25rem', borderTop: '1px solid var(--border)' }}>
                            ...
                            <button type="button"
                                disabled={!subSelectedPlanId || isSubscribing}
                                onClick={async () => {
                                    if (!subSelectedPlanId) return alert('Selecione um plano.')
                                ...
                                style={{ padding: '0.6rem 1.5rem', borderRadius: '10px', border: 'none', background: 'var(--gradient-primary)', color: 'white', cursor: 'pointer', fontWeight: 700, fontSize: '0.95rem', opacity: (!subSelectedPlanId || isSubscribing) ? 0.5 : 1 }}
                    ...
                    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', background: 'rgba(0,0,0,0.5)' }} onClick={() => setEditingSubId(null)}>
                        <div style={{ background: 'var(--bg-primary)', padding: '2rem', borderRadius: '20px', width: '100%', maxWidth: '520px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }} onClick={e => e.stopPropagation()}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
                                <h3 style={{ fontSize: '1.2rem', fontWeight: 700 }}>⚙️ Ajustar Mensalidade — {plan?.name}</h3>
                            ...
                            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', marginTop: '1.5rem', paddingTop: '1.25rem', borderTop: '1px solid var(--border)' }}>
                                <button type="button" onClick={() => setEditingSubId(null)} style={{ padding: '0.6rem 1.25rem', borderRadius: '10px', border: '1px solid var(--border)', background: 'var(--bg-tertiary)', color: 'var(--text-secondary)', cursor: 'pointer', fontWeight: 600 }}>Cancelar</button>
                                <button
                                    onClick={async () => {
                                        // Constrói creditSchedules para enviar
                                        const creditSchedules = Object.keys(subSchedules).map(serviceId => ({
                                    ...
                                    style={{ padding: '0.6rem 1.5rem', borderRadius: '10px', border: 'none', background: 'var(--gradient-primary)', color: 'white', cursor: 'pointer', fontWeight: 700, fontSize: '0.95rem' }}
                                >
                                    Salvar Alterações
                                </button>
                            </div>
