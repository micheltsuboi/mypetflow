...
export async function getLabRequests(filters?: { pet_id?: string; status?: string }) {
    try {
        ...
        let query = supabaseAdmin
            .from('lab_requests')
            .select('*, pets(name, species, breed, birth_date, gender), customers(name, phone_1), lab_exams(name, category), veterinarians(name, crmv)')
            .eq('org_id', profile.org_id)
        ...
        const { data: request, error: reqErr } = await supabaseAdmin
            ...
            .select(`
                *,
                ...
                pets(id, name, species, breed, gender, birth_date),
